...
The `:name` in the URL is the **source branch** (the branch being merged). The request body specifies the **target** and **strategy**:
...
- `target` — Target branch to merge into (optional, defaults to the repo's default branch)
...
```http
POST /v1/repos/my-app/branches/feature-x/merge
```
