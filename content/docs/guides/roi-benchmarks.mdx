...
> **Write throughput: 15,000 commits/hour vs GitHub's ~370. That's 40x more capacity.** GitHub's secondary rate limit (80 content-creating requests/minute ≈ 4,800/hour) means an AI agent hits the wall after ~370 ten-file commits per hour (13 calls each). Coregit handles 15,000 — your agent never waits.
...
GitHub: 5,000 requests/hour primary limit, plus a secondary rate limit of 80 content-creating requests/minute (~4,800/hour). At 13 calls per 10-file commit, that's ~370 commits/hour before you hit the ceiling.

Coregit: 15,000 requests/hour per key, 50,000/hour per org. At 1 call per commit, that's 15,000 commits/hour — **~40x more throughput** than GitHub.
...
- **For platform teams:** compare rate limits (~370 commits/hr on GitHub vs 15,000/hr on Coregit)
